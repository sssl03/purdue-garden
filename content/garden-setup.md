...
1. **quartz**:
	- get [quartz](https://quartz.jzhao.xyz/), and follow the [installation](https://quartz.jzhao.xyz/getting-started/installation) steps. keep this in mind:
		...
		- i like to call this project `garden`. so, on my computer, the folder in which quartz sits is usually called `garden`. (you may call it `quartz`, or `website`, or `blog`, or something else. but in this document, whenever i refer to a folder called `garden`, i am referring to the main folder into which quartz was cloned.)
		- (optional) instead of using the default `v5` branch, create a `main` branch to track all changes (customisations to quartz and obsidian, and content updates) to the repo. this is a _me_ thing ; i like to segregate things ; it creates extra work, but usually helps me recover from mistakes or tidy-up messes better. if you're new to coding, git, and what-not, skip this; just commit to quartz's default `v5` branch, and ignore any instructions (in this document) where i ask you to replace `v5` by `main`. 
...
2. **obsidian**:
	...
	- create a new vault, like so:
		- go to `file` > `open vault..`. in the modal that opens, select `create new vault`. on the next step, 
			- for `vault name`: type `content`. 
			- and for `location`: press `browse` and select the main folder into which quartz was installed, i.e., `garden`. (be careful not to select the `garden/content` folder here.)
		- this will create a `.obsidian` folder inside the existing `garden/content` folder; and the vault will open in obsidian.
	- customise obsidian vault's preferences:
		- (these customisations are a *me* thing, and very-much optional. please stick to obsidian's vanilla setup.)
			- set locations for different files:
				- page templates: `_templates` (in the `templates` section)
				...
				- attachments: `_attachments` (in the `files & links` section)
				- regular notes: `notes` (in the `files & links` section)
				- new file location for daily-notes: `log` (in the `daily notes` section)
			- whenever we customise a file-location, we need to create the corresponding folders: `_templates`, `_attachments`, `log` and `notes`. (if correctly-named folder don't exist, new notes just go into the root folder, i.e., `garden/content`.)
			- in `daily notes`,
		...
		- under `files & links`, set deleted files to move to obsidian's `.trash` folder (to make them easier to recover in-case of accidental deletion). 
			- also: add `.trash` to `.gitignore`

### write.

at this point, this is good to know: 
- most files in the `content` folder will be `.md` files. 
...
- serve (preview) the website: 
	- open the terminal at the `garden` directory.
